...
```
浏览器         AuthController    AuthService    UserService    PasswordEncoder    JwtUtil        PG(pgsql)
...
| 动物档案管理         [+新增] | 新增动物档案                   |
+----------------------------+--------------------------------+
| [名字前缀______] [全部类型 v] | 名字* [____________]            |
| [搜索] [重置]                | 类型* [请选择 v]  CAT/DOG      |
|                              | 常驻区域* [_____________]      |
| 1. 奶牛猫 CAT 图书馆草坪   ✎🗑| 封面照片 [选择文件]            |
| 2. 橘猫   CAT 西区食堂     ✎🗑|   (未选择图片)                 |
| 3. 黑狗   DOG 东门         ✎🗑| 仅 jpg/png/gif/webp,<5MB      |
| 4. 白猫   CAT 教学楼后     ✎🗑| [提 交]  [重置/取消编辑]      |
|                              |                                |
|   < 上一页  第 1 页  下一页>|                                |
+----------------------------+--------------------------------+
```
...
课设环境提供的是 PostgreSQL 18 实例,没有 MySQL 服务。MyBatis-Plus 本身不绑定数据库方言,只要求数据源驱动类与方言匹配,改造做两件事:
