...
完整 DDL 见 `project/sql/init.sql`,关键表结构如下(外键、字符集、注释已按 MySQL 8 规范补全):

**`user` 表**——系统账号,学生/管理员共用:
...
> `idx_type_deleted` 是图鉴列表的核心索引(`WHERE type=? AND deleted=0`),把"等值过滤列"放第一位,"软删过滤列"放第二位,既能命中 type 单条件,也能命中 `type + deleted` 组合条件。

**`check_in` 表**——课程设计核心一对多关系的"多方":
...
CREATE TABLE check_in (
  ...
  CONSTRAINT fk_checkin_animal FOREIGN KEY (animal_id) REFERENCES animal(id)
                  ON DELETE RESTRICT,
...
> 复合索引 `(animal_id, created_at)` 顺序特意写成"等值列在前,排序列在后",这样时间轴 `WHERE animal_id=? ORDER BY created_at DESC` 能**直接利用索引的有序性**,EXPLAIN 不会出现 `Using filesort`(实测见 §4.4.3)。`content VARCHAR(500)` 选用 VARCHAR 而非 TEXT,是因为 500 字以内完全够一条打卡,VARCHAR 走 `utf8mb4` 不需要溢出页,查询性能比 TEXT 好。
...
- `code = 0` 成功
- 非 0 业务错误码（1001 用户名已存在、1002 用户名或密码错误、2001 动物不存在、4001 未登录、4003 无权限、9999 服务器内部错误）
...
```
浏览器         AuthController    AuthService    UserService    PasswordEncoder    JwtUtil        MySQL
   ...
   │                  │                  │── matches(plain, hash) ─────────▶│
   │                  │                  │   ◀── true/false ─────────────────│
   │                  │                  │  [若 false 抛 INVALID_CREDENTIALS]  │
   │                  │                  │── generate(userId, name, role) ──▶│
   │                  │                  │   ◀── token (eyJ...) ──────────────│
   │                  │   ◀── LoginResp ──│
   │   ◀── 200 {code:0, data:{token,...}} │
   │── 写 localStorage["animal_token"]   │
...
```
浏览器(ADMIN)   AnimalController   AnimalService   FileStorageService   MySQL   本地磁盘
...
**索引命中验证**:开发期在本地 MySQL 8.0 上跑 `EXPLAIN`(本节贴出来的输出是 dev 环境真实 EXPLAIN 结果):
...
实事求是地说,本系统离"生产可用"还有不少距离,以下三条是我**主动承认**的不足,不做粉饰:
